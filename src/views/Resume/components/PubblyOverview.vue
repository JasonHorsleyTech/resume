<template>
  ...
  >
    <template v-slot:overview>
      <div class="space-y-2">
        <p>
          Pubbly was an experimental offshoot of an established New York book
          publishing house. It was an attempt to quickly and cheaply digitize
          their owned intellectual properties, and bring childrens' education
          into the modern age.
        </p>
        <p>
          Our main focus was on the Global Learning XPrize competition, an Elon
          Musk charity project to increase access to education in historically
          empoverished countries. Our open sourced entry allowed for rapid
          creation of interactive e-book templates, which could then be
          duplicated, translated, packaged and delivered via custom android
          tablet system images.
        </p>
        <p>
      ...
      <div class="grid grid-cols-3">
        <list
          v-for="({ category, items }, index) in [
            {
              category: 'Frontend',
              items: ['Javascript', 'jQuery', 'Webix'],
            },
            {
              category: 'Backend',
              items: ['Laravel', 'Symphony', 'Python', 'Livecode'],
            },
            {
              category: 'Misc',
              items: ['MySQL', 'Cordova', 'AWS', 'Shell'],
            },
          ]"
          :items="items"
          :key="index"
